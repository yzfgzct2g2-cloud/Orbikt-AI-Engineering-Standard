...
1. **Pin an edition.** Choose the OAES edition from [CHANGELOG.md](../CHANGELOG.md) and
   ...
   state (PRJ-02, EVO-08).
2. **Run Discovery before anything else.** For each delivery project, the
   [Discovery Framework](../31-discovery/discovery-specification.md) precedes and
   produces the charter: classification, goal alignment, five to ten material questions
   (or a countersigned confidence declaration), constraints, risks, and a human approval
   gate (DSC-01, DSC-12). The
   [Discovery Record Template](../60-templates/tmpl-011-discovery-record.md) carries it.
3. **Write the charter from Discovery's output.** Fill the
   [Project Charter Template](../60-templates/tmpl-008-project-charter.md) — purpose,
   scope, roles, data classes, declared adapters (ADP-07), intended conformance level.
   The [example charter](../61-examples/example-project-charter.md) shows a completed
   one (PRJ-01).
4. **Name humans.** Fill the role register: at least one Maintainer, Stewards for what
   you will govern (ORG-02). Every later step assumes accountability exists.
5. **Instantiate the information layers.** Create your knowledge index (KNO-08), decide
   memory retention rules (MEM-06), and set context budgets for your first task classes
   (CTX-07). Start small; these grow with use.
6. **Declare your agents.** For every automated helper that will act on artifacts, write
   ...
   (AGT-03).
7. **Define your first workflows.** At minimum, your change workflow and your review
...
8. **Wire in validation.** Adopt the layer-63 rule sets for your OAES-structured
   artifacts so structural review is free from day one (QUA-03).
9. **Declare conformance honestly.** Start at Level 1 (artifacts exist and validate),
...
| 1.1.0 | 2026-07-08 | OAES Standards Committee | Added the Discovery step and adapter declaration (Foundation edition 1.1). |
